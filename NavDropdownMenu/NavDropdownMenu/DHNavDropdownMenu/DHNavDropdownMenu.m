...
@interface DHNavDropdownMenu () <UITableViewDataSource, UITableViewDelegate>
...
@implementation DHNavDropdownMenu

- (instancetype)init {
    
    self = [DHNavDropdownMenu buttonWithType:UIButtonTypeCustom];
    if (self) {
...
- (void)menuAction:(DHNavDropdownMenu *)sender {
...
- (void)setDataSource:(id<DHNavDropdownMenuDataSource>)dataSource {
    _dataSource = dataSource;
    [self setTitle:self.titleArray.firstObject forState:UIControlStateNormal];
}

- (void)setDelegate:(id<DHNavDropdownMenuDelegate>)delegate {
    _delegate = delegate;
