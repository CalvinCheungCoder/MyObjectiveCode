...
- (void)closeThePage{
    ...
    for (UserData *temp in arr) {
        ...
        if ([_word.text isEqualToString:temp.word] && [_desText.text isEqualToString:temp.des]) {
            ...
            return;
        }else if ([_word.text isEqualToString:temp.word]){
            
            [self.manger updateDataWith:data];
            [self.navigationController popViewControllerAnimated:YES];
            return;
        }else if ([_desText.text isEqualToString:temp.des]){
            
            [self.manger updateDataTwoWith:data];
            [self.navigationController popViewControllerAnimated:YES];
            return;
        }else{
